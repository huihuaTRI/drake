...
class DoorHingeTester {
 public:
  explicit DoorHingeTester(std::unique_ptr<DoorHinge<double>> door_hinge) {
    door_hinge_ = std::move(door_hinge);
  }
 ...
 private:
  std::unique_ptr<DoorHinge<double>> door_hinge_;
};
...
class DoorHingeTest : public ::testing::Test {
 public:
  ...
  DoorHingeTester CreateDoorHingeTester(const DoorHingeConfig& config) {
    auto door_hinge = std::make_unique<DoorHinge<double>>(joint(), config);
    return DoorHingeTester(std::move(door_hinge));
  }
